...
def get_password(account: Account, *, allow_prompt: bool = True) -> str:
    ...
    if not allow_prompt:
        raise AuthError(
            f"No password in keyring for {account.keyring_service!r} / "
            f"{account.username!r}. Run `calmcp login` (Phase 3) or set it in your keyring."
        )
    if not sys.stdin.isatty() and platform.system() != "Darwin":
        raise AuthError(
            f"No password in keyring for {account.keyring_service!r} and no TTY to prompt on."
        )
